...
say "AFTER: cloud=$AC  transit(no-token)=$TR  segment-pull(no-token)=$SEG  transit(analysis-token)=$OK_TR"
if [ "$AC" = active ] && [ "$TR" = 401 ] && [ "$SEG" = 401 ] && [ "$OK_TR" = 200 ]; then
  say "RESULT: PASS. gw_event schema was logged: journalctl -u $SVC | grep 'gw_event columns' (paste it to finalize backfill)."
  echo "======================================================================"
